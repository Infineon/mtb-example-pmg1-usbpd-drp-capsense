...
/* Flag to indicate fast scanning of capsense widgets */
    ...
    if (ptrSliderPosition->numPosition == ONE_TOUCH)
    {
        ...
        if (ptrPdStackContext->dpmConfig.contractExist == true)
        {
            ...
            if (ptrPdStackContext->dpmConfig.curPortRole == CY_PD_PRT_ROLE_SOURCE)
            {
                ...
                {
                    /* Update Source Caps - 5V */
                    Cy_PdStack_Dpm_UpdateSrcCapMask (ptrPdStackContext, FIXED_5V);
                }
                else if( (position > ( 1 * STEP_SIZE)) && (position <= ( 2 * STEP_SIZE)))
                {
                    /* Update Source Caps - 5V and 9V */
                    Cy_PdStack_Dpm_UpdateSrcCapMask (ptrPdStackContext, FIXED_9V_AND_FIXED_5V);
                }
                else if( (position > ( 2 * STEP_SIZE)) && (position <= (3 * STEP_SIZE)))
                {
                    /* Update Source Caps - 5V and 12V */
                    Cy_PdStack_Dpm_UpdateSrcCapMask (ptrPdStackContext, FIXED_12V_AND_FIXED_5V);
                }
                else if( (position > ( 3 * STEP_SIZE)) && (position <= ( 4 * STEP_SIZE)))
                {
                    /* Update Source Caps - 5V and 15V */
                    Cy_PdStack_Dpm_UpdateSrcCapMask (ptrPdStackContext, FIXED_15V_AND_FIXED_5V);
                }
                else if( (position > ( 4 * STEP_SIZE)) && (position <= ( 5 * STEP_SIZE)))
                {
                    /* Update Source Caps - 5V and 15V */
                    Cy_PdStack_Dpm_UpdateSrcCapMask (ptrPdStackContext, FIXED_20V_AND_FIXED_5V);
                }
                else
                {
                    /* Do nothing */
                ...
                /* Send Source Cap Change PD message */
                Cy_PdStack_Dpm_SendPdCommand(ptrPdStackContext, CY_PDSTACK_DPM_CMD_SRC_CAP_CHNG,
                                                                                 NULL, false, NULL);
            }
            /* Sink */
            else
